...
- `항진 명제(tautology)` : 합성 명제에서 그 명제를 구성하는 단순 명제들의 진리값에 관계없이 진리값이 항상 참의 값을 가지는 명제

- `모순 명제(contradiction)` : 합성 명제에서 그 명제를 구성하는 단순 명제들의 진리값에 관계없이 진리값이 항상 거짓의 값을 가지는 명제

항진 명제의 부정은 모순 명제이고, 모순 명제의 부정은 항진 명제이다.

<br/>

## 2.4 논리적 동치 관계

두 개의 명제 p, q의 쌍방 조건 p↔q가 항진 명제이면, 두 명제 p,q는 `논리적 동치(logical equivalence)`라 하고, p≡q 또는 p⇔q라고 표시한다.

즉, 명제 p와 q는 같은 논리값을 가진다는 의미이다.

- 두 명제가 논리적 동치일 경우는 두 명제의 논리값이 서로 같음
- 하나의 명제가 다른 명제를 대신하여 사용할 수 있음
- 복잡한 명제를 간단한 명제로 만들 때 간소화할 수 있음
...
주어진 명제가 참인 것을 바탕으로 새로운 명제가 참이 되는 것을 유도해내는 방법을 `추론(Argument)`이라고 한다.

- 주어진 명제들인 p1, p2, ..., pn을 `전제(premise)`라 한다.
- 새로이 유도된 명제 q를 `결론(conclusion)`이라고 한다.

여기서 주어진 전제가 참이고 결론도 참이 추론을 `유효 추론(valid argument)`라 하고, 추론이 거짓이면 `허위 추론(fallacious argument)`이라고 한다.

전제를 p1, p2, ..., pn이라 하고 결론을 q라 할 때, 추론에 대한 것을 수학적 식으로 표시하면
...
변수의 값에 따라서 참 혹은 거짓이 될 수 있는 명제를 `p(x)`로 표시하고, p(x)를 변수 x에 대한 `명제 술어(propositional predicate)`라 한다.

명제 술어에 대한 논리를 `술어 논리(predicate logic)`라 한다.

술어를 나타내는 방법 중 변수의 범위를 한정시키는 것을 `술어 한정자(predicate quantifier)`라 한다.

- '모든 것에 대하여(for all)' 와 '존재한다(there exist)'의 두 가지가 있다.
- '모든 것에 대하여'
  - 전체 한정자(universal quantifier)
  - 기호 ∀를 사용한다.
  - '모든 x에 대하여 p(x)는 참이다.' = `∀xp(x)`
  - ∀xp(x)가 참이 되기 위한 필요충분조건 : 술어 p(x)가 x의 전체 집합 U에 대해 성립해야 한다.
- '존재 한다'
  - 존재 한정자(existential quantifier)
  - 기호 ∃를 사용한다.
  - '어떤 x에 대하여 p(x)가 참인 x가 존재한다' = `∃xp(x)`
  - ∃xp(x)가 참이 되기 위한 필요충분조건 : 전체 집합 U 안에 p(x)를 만족시키는 x가 적어도 한 개 존재해야 한다.

#### 예시

- `∃x p(x, y)` : p(x, y)가 성립하는 x가 존재한다.
- `~(∀x p(x))` : 모든 x에 대하여 p(x)가 성립하는 것은 아니다.
- `∃y (∀x p(x, y))` : 모든 x에 대하여 p(x, y)가 성립하는 y가 존재한다.
